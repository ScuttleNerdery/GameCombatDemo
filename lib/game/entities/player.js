...
EntityPlayer = ig.Entity.extend({
  ...
  online_refreshing: false,
  curr_player_id: getParameterByName('player_id'),
  ...
  abilities_locked: false,
    ...
    // Update player in the online world
    if (char_sprite.online_refreshing == false) {
      char_sprite.online_refreshing = true;
      ajax = new ig.Ajax();
      console.log();
      ajax.post( "/online/socket.php", {player_id: char_sprite.curr_player_id, player_x: char_sprite.pos.x, player_y: char_sprite.pos.y}, function(result) {

        // TODO - move players based on the data above
        var other_players = JSON.parse(result);
        for (player_id in other_players) {
          if (player_id != char_sprite.curr_player_id) {

            //char_sprite.pos.x = other_players[player_id].x_coor;
            //char_sprite.pos.y = other_players[player_id].y_coor;

            var other_player = ig.game.getEntitiesById( EntityOnlinePlayer, player_id )[0];
            if (other_player) {
              other_player.pos.x = parseInt(other_players[player_id].x_coor)
              other_player.pos.y = parseInt(other_players[player_id].y_coor)
            }
            else {
              var other_player_settings = { id: player_id };
              var other_player = ig.game.spawnEntity(EntityOnlinePlayer, parseInt(other_players[player_id].x_coor), parseInt(other_players[player_id].y_coor), other_player_settings);
            }
          }
        }

        char_sprite.online_refreshing = false;
      });
    }
